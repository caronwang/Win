...
//创建一个玩家对象
func NewPlayer(conn IConnection) *Player {
	//生成一个PID
	IdLock.Lock()
	id := PidGen
	PidGen++
...
func (p *Player) Leave() {
	fmt.Println("player id = ", p.Pid, " is offline...")

	players := p.GetSurroundingPlayers()

	for _, player := range players {
		fmt.Println("GetSurroundingPlayers: pid= ", player.Pid)
		//组建MsgId0 proto数据
		data := &pb.SyncPid{
			Pid: p.Pid,
		}
